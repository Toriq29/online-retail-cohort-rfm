...
Berdasarkan total 5,878 pelanggan (tercatat dari total distribusi pada Monetary/Frequency), segmentasi menunjukkan bahwa kelompok Champions merupakan kontributor terbesar dengan nilai transaksi mencapai $19,892,859$, diikuti oleh segmen berisiko tinggi yaitu Hibernating ($11.95M) dan At Risk ($11.95M). Tingginya angka pada segmen At Risk dan Hibernating menunjukkan adanya potensi churn pelanggan bernilai tinggi yang cukup signifikan jika tidak segera ditangani, sementara segmen New Customers masih tergolong sangat kecil ($765K).

Distribusi RFM juga menunjukkan pola right-skewed yang sangat tajam, di mana mayoritas pelanggan (2,971 customer) memiliki Recency transaksi yang sangat baru (bin 0). Namun, pada grafik Frequency dan Monetary, sebanyak 5,824 pelanggan (~99% dari keseluruhan customer base) terkonsentrasi di bin terendah (transaksi sangat jarang dan nilai spending rendah). Hal ini menandakan bahwa mayoritas pelanggan adalah occasional buyer, sementara nilai transaksi keseluruhan didorong oleh segelintir pelanggan utama (whale customers). Pola ini menegaskan pentingnya penggunaan skor persentil/kuantil dalam analisis RFM agar segmentasi tidak bias oleh data pencilan (outlier).
